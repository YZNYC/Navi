...
import { z } from 'zod';
import api from '../../lib/api'; // <-- ADICIONE esta linha (ajuste o caminho se necessário)
...
const cadastroSchema = z.object({
  ...
  senha: z.string().min(3, "A senha precisa ter no mínimo 3 caracteres"),
});


// --- Componente do Formulário ---
const AuthForm = ({ isLoginView, toggleView }) => {
...
// Na sua função onSubmit
const onSubmit = async (data) => {
    setIsSubmitting(true);
    clearErrors("apiError");
    
    // As URLs agora são relativas à baseURL que definimos ('http://localhost:3000')
    const url = isLoginView ? `/auth/login` : `/usuarios`; 
    
    let apiData = { ...data };
    if (!isLoginView) { 
        apiData.nome = `${data.nome} ${data.sobrenome}`.trim();
        // Importante: removemos o 'sobrenome' pois a API não espera por ele
    ...
    return (
        <div className="p-6 sm:p-10 w-full flex flex-col">
            <div className="text-center mb-8">
                ...
                <h1 className="mt-4 text-2xl sm:text-3xl font-bold text-gray-800 tracking-tight">
                    {isLoginView ? 'Bem-vindo de Volta' : 'Crie Sua Conta'}
                </h1>
                <p className="mt-1 text-sm text-gray-500">
                    {isLoginView ? 'Acesse seu painel' : 'Rápido e sem complicações'}
                </p>
            </div>
            <form onSubmit={handleSubmit(onSubmit)} noValidate className="space-y-4">
                ...
                {!isLoginView && <SoftInput id="telefone" label="Telefone (Opcional)" type="tel" register={register('telefone')} error={errors.telefone} />}
                <SoftInput id="email" type="email" label="Endereço de Email" register={register('email')} error={errors.email} />
                <SoftInput id="senha" type={isPasswordVisible ? 'text' : 'password'} label="Senha" register={register('senha')} error={errors.senha} hasIcon={true} isPasswordVisible={isPasswordVisible} togglePasswordVisibility={() => setIsPasswordVisible(!isPasswordVisible)} />
                ...
                {isLoginView && (
                    <div className="flex items-center justify-between pt-1 text-sm">
                        ...
                        <button type="button" className="font-semibold text-yellow-600 underline-grow">Esqueci a senha</button>
                    </div>
                )}

                {errors.apiError && <p className='text-red-500 text-sm text-center pt-2'>{errors.apiError.message}</p>}
                <div className="pt-4">
                  <button type="submit" disabled={isSubmitting} className="main-button">{isSubmitting ? 'Aguarde...' : (isLoginView ? 'Entrar' : 'Cadastrar')}</button>
                </div>
            </form>
            <div className="my-6 flex items-center gap-4">
                <div className="h-px bg-gray-200 flex-1"></div><span className="text-xs text-gray-400 font-medium">ou</span><div className="h-px bg-gray-200 flex-1"></div>
            </div>
            ...
            <div className="mt-8 text-center text-sm text-gray-500">
                ...
                <button onClick={onToggleView} className="font-semibold text-yellow-600 underline-grow ml-1">{isLoginView ? 'Cadastre-se' : 'Faça login'}</button>
            </div>
        </div>
    );
};
...
// --- Componente do Input ---
function SoftInput({ id, label, type = 'text', register, error, hasIcon = false, isPasswordVisible, togglePasswordVisibility }) {
    return (
...
function DynamicBackground({ src }) {
    ...
    return isVideo ? <video className="absolute inset-0 w-full h-full object-cover -z-10" autoPlay loop muted><source src={src} type={`video/${src.split('.').pop()}`} /></video> : <Image src={src} alt="background" fill className="absolute inset-0 object-cover -z-10 opacity-40" />;
}
...
export default function AuthPage() {
    ...
    useLayoutEffect(() => {
        // Alturas fixas estimadas para cada visualização
        const loginHeight = '720px'; // Ajustado para incluir o 'Lembrar de mim'
        const registerHeight = '870px';
        setContainerHeight(isLoginView ? loginHeight : registerHeight);
    }, [isLoginView]);

    const toggleView = () => {
        if (isAnimating) return;
        setIsAnimating(true);
        setTimeout(() => setIsLoginView(prev => !prev), 700);
    ...
    return (
        ...
        <main className="min-h-screen flex items-center justify-center bg-[#F7F4F0] p-4 font-sans overflow-hidden">
            ...
            <div className="relative w-full max-w-lg bg-white/60 backdrop-blur-lg border border-white/30 rounded-3xl shadow-2xl overflow-hidden transition-[height] duration-500 ease-in-out" style={{ height: containerHeight }}>
                <div className={`absolute inset-0 transition-opacity duration-300 ${isAnimating ? 'opacity-0' : 'opacity-100 delay-[800ms]'}`}>
                    <AuthForm isLoginView={isLoginView} toggleView={toggleView} />
                </div>
