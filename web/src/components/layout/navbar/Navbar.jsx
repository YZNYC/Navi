...
const ThemeToggleButton = ({ theme, toggleTheme }) => (<button onClick={toggleTheme} className="p-2 rounded-full border border-slate-200 dark:border-slate-700 hover:border-amber-500 transition-colors" aria-label="Trocar Tema">{theme === "dark" ? <Sun className="w-6 h-6 text-slate-400" /> : <Moon className="w-6 h-6 text-slate-500" />}</button>);
...
const FullscreenOverlay = ({ title, onClose, children }) => (
  <div className="fixed inset-0 z-40 backdrop-blur-md bg-black/40 dark:bg-black/50 flex flex-col lg:hidden">
    <div className="flex items-center justify-between bg-white/80 dark:bg-slate-900/80 backdrop-blur-md p-4 shadow-md">
      ...
      <button onClick={onClose} className="p-1 rounded-full hover:bg-slate-200 dark:hover:bg-slate-700 transition"><X className="w-6 h-6 text-slate-800 dark:text-white" /></button>
    </div>
    <div className="flex-1 flex flex-col items-center justify-center text-slate-600 dark:text-slate-400">{children}</div>
  </div>
);
...
const ProfileButton = ({ user, logout, isOpen, setIsOpen, isMobile = false }) => { if (!user || !user.nome) return null; const userNameForAvatar = user.nome.replace(" ", "+"); return ( <div className={`relative ${isMobile ? "" : "ml-2"} group`}><button onClick={setIsOpen} className="flex items-center gap-2"><img src={user.url_foto_perfil || `https://ui-avatars.com/api/?name=${userNameForAvatar}&background=fb923c&color=fff`} alt="Avatar" className="w-10 h-10 rounded-full border-2 border-slate-300 dark:border-slate-600 group-hover:border-amber-500 transition"/><div className={`flex items-center gap-1 ${isMobile ? "" : "hidden md:flex"}`}><span className="text-sm font-medium text-slate-800 dark:text-white">{user.nome}</span><ChevronDown className={`w-4 h-4 text-slate-400 transition-transform ${isOpen ? "rotate-180" : ""}`} /></div></button>{isOpen && <ProfileDropdown logout={logout} />}</div> );};
const LoginButtons = () => ( <div className="flex items-center gap-4"><a href="/" className="text-sm font-medium text-slate-700 dark:text-slate-300 hover:text-amber-500 dark:hover:text-amber-400">Entrar</a></div> );
const ProfileDropdown = ({ logout }) => ( <div className="absolute top-full right-0 mt-3 w-48 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-lg py-2 z-50"><Link href="/perfil" className="flex items-center gap-3 px-4 py-2 text-sm text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700"><UserCircle className="w-5 h-5" />Meu Perfil</Link><div className="h-px my-2 bg-slate-500/30"></div><button onClick={logout} className="w-full flex items-center gap-3 px-4 py-2 text-sm text-red-600 dark:text-red-400 hover:bg-slate-100 dark:hover:bg-slate-700"><LogOut className="w-5 h-5" />Sair</button></div> );